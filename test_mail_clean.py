...
def cargar_grupos(ruta_archivo: Path) -> dict:
...
def eliminar_filas(ruta_archivo: Path, df_completo: pd.DataFrame, sucursal) -> pd.DataFrame:
...
if __name__ == "__main__":
    def _main():
        ...
        print(f"[1] Ruta: {ruta}")
        print(f"[2] Existe el archivo: {ruta.exists()}")

        df_principal = pd.read_excel(ruta, sheet_name=settings.NOMBRE_HOJA)
        print(f"[3] Filas cargadas: {len(df_principal)}")
        print(f"[4] Columnas: {list(df_principal.columns)}")

        grupos_cargados = cargar_grupos(ruta)
        print(f"[5] Grupos encontrados: {len(grupos_cargados)}")
        print(f"[6] Claves: {list(grupos_cargados.keys())}")
